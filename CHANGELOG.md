...
# 1.0.0
- Documented that year `0` and negative years are supported across calendar navigation, events, Frontmatter imports, weather, moons, and recurrence calculations.
- Added a Style Settings slider for light-theme Timeline box brightness.
- The brightness adjustment affects only Timeline Style entries with a custom box background and does not affect dark Obsidian themes.
- Added optional Style Settings color pickers for normal calendar day cells, the calendar header, and the calendar content/scroller area.
- Added a themed Style Settings color for month names in Calendar View.
- Added a calendar-specific fixed weekday-label abbreviation setting for Calendar View.
- Weekday labels can now use full names or a fixed length of 1, 2, 3, or 4 characters.
- Removed width-based automatic weekday abbreviation, so weekday labels no longer depend on window resizing, view refreshes, or changing the displayed year.
